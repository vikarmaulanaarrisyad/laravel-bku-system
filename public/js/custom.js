...
$("ul.nav-sidebar a")
    .filter(function () {
        if (this.href) {
            // Check if current URL matches the menu link or starts with it
            return this.href == url || url.href.indexOf(this.href) == 0;
        }
    })
    .addClass("active"); // Add 'active' class to the matching menu item

// For the treeview (submenu)
$("ul.nav-treeview a")
    .filter(function () {
        if (this.href) {
            // Check if current URL matches the submenu link or starts with it
            return this.href == url || url.href.indexOf(this.href) == 0;
        }
    })
    .parentsUntil(".nav-sidebar > .nav-treeview") // Find all parent elements of the treeview
    .addClass("menu-open") // Open the submenu
    .prev("a") // Target the parent link of the submenu
    .addClass("active"); // Add 'active' class to the parent menu item
